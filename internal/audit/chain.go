...
func (c *Chain) Verify() error {
	...
	prev := ""
	lastRevision := map[string]int{}
	for _, line := range splitLines(b) {
		var ev Event
		if json.Unmarshal(line, &ev) != nil {
			return fmt.Errorf("invalid audit event")
		}
		if previous, ok := lastRevision[ev.CaseID]; ok && ev.Revision <= previous {
			return fmt.Errorf("audit revision out of order")
		}
		lastRevision[ev.CaseID] = ev.Revision
		d := ev.Digest
		ev.Digest = ""
		raw, _ := json.Marshal(ev)
		sum := sha256.Sum256(raw)
